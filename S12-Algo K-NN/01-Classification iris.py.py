...
df_iris = pd.read_csv("IRIS-dataset.csv")
print(df_iris)


# Sélection des données à afficher :
# ----------------------------------
longueur_petale = df_iris["petal_length"]
largeur_petale = df_iris["petal_width"]
espece = df_iris["species"]


# Ajout du nouvel individu :
# --------------------------
iris_inconnue = {"longueur_pétale": 2.5, "largeur_pétale": 0.75}


# Classification du nouvel individu :
# -----------------------------------
zip_iris = list(zip(longueur_petale, largeur_petale)) # Mise en forme du jeux de données des longueurs et largeurs

k = 3 # Nombre de voisins validant la classification d'un nouveau individu
mode1 = Knn(n_neighbors = k) # Instance d'un modèle

mode1.fit(zip_iris, espece) # Entrainement du modèle

classification = mode1.predict([(iris_inconnue["longueur_pétale"], iris_inconnue["largeur_pétale"])])
print(f"L'iris inconnue est classée (avec k={k}) comme : {classification[0]}")
