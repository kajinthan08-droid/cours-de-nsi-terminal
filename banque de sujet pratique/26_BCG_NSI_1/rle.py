...
def decodage_rle(liste_rle):
    ...
    while i < len(liste_rle):
        compte = liste_rle[i]      # nombre de répétitions
        valeur = liste_rle[i + 1]  # valeur à répéter
        for j in range(compte):
            liste_octets.append(valeur)
        i += 2                     # on avance de 2 (une paire à la fois)
    return liste_octets
                             
print(decodage_rle([2, 255, 1, 0, 3, 255]))
# Attendu : [255, 255, 0, 255, 255, 255]
